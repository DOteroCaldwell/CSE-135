...
/**
 * CSE 135 HW4 Part 4 / HW5 — "Page load cost", the performance report body.
 *
 * GUIDING QUESTION
 ...
 *
 * This file is a FUNCTION, not a page. reports/page-load-cost.php renders it live
 * and saved/save.php renders it into a snapshot; there is one copy of the report.
 */
...
function render_report_page_load_cost(Filters $f): void
{
...
$thin         = $n < Coverage::MIN_PAGEVIEWS;
    ...
    if ($opp->isEmpty()) {
        report_empty('This report needs at least one pageview with navigation timing. '
            . 'Adjust the filters on the dashboard, or generate traffic against the '
            . 'instrumented site.');
        return;
    }
?>
...
?>
    ...
    <section>
      ...
      <ul>
<?php foreach (PAGE_LOAD_COST_REMEDIES[$winner] ?? [] as $r): ?>
        <li><?= e($r) ?></li>
<?php endforeach; ?>
      </ul>
      <p class="card-question">
